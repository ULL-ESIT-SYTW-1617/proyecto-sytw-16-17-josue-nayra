          ...
          if(resolve2 != false){
             	heroku.post('/apps', {body: {name: resolve1}})
                    ...
                    switch (tipo_bd)
                    {
                      ...
                      break;
                      case 'dropbox':
                        console.log(" - Creando directorio .dropbox");
                        fs.mkdir(path.join(process.env.HOME,'.dropbox'), (err)=>
                        {
                          if(err)
                            reject(err);
                          ...
                          inquirer.prompt(schema).then((respuestas) =>
                          {
                              console.log("  - Creando fichero ~/.dropbox/dropbox.json");
                              config.token_dropbox = respuestas.token_dropbox;
                              config.link_bd = respuestas.link_bd;
                              jsonfile.spaces = 10;
                              jsonfile.writeFileSync(path.join(process.env.HOME,'.dropbox','dropbox.json'),config,{spaces: 10});
                              result(respuesta1.git_url);
                          });
